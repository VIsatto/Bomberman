...
short Backup(DADOS players[N_RANKING]);
...
short Backup(DADOS players[])
{
    ...
    else
    {
        ...
        else
        {
            ...
            do
            {
                if((fread(&players[i],sizeof(DADOS),1,arquivo) == 1))
                {
            ...
            }while(i<N_RANKING && !feof(arquivo));
            printf("\n\nBACKUP REALIZADO\n");
            fclose(backup);
